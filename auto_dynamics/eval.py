...
class InferenceModel(object):
  ...
  def __call__(self, ex):
    ...
    ds, original_input = self.midi_file_to_dataset(ex)
      ...
      for i, num in enumerate(arr):
        ...
        if (i % 3 == 0):
          pitch_predictions.append(int(num))
            ...
            velocity_predictions.append(int(num))
            out+=(int(num)-original_input[i][3])**2
    ...
    return nns, out/(sum(target_lengths)//3)


  def midi_file_to_dataset(self, midi_file):

    length = self.inputs_length
    target_length = self.outputs_length 
    ...
    def get_input():
      ...
      orig_input = np.zeros((len(notes), 4)).astype(float)
      length_index = 0
      for note in notes:
          ...
          orig_input[length_index][3] = velocity
...
def main(args):
  ...
  # piano_model = InferenceModel('/home/grace/model/mse1hot.512/checkpoint_2160000')

  print(f"evaluating model {model_path}{checkpoint_path}")
  filename = '/home/grace/datasets/maestro/v3.0.0/orig/maestro-v3.0.0/maestro-v3.0.0.csv'
  data = []
  sum_loss = 0
  num_validation = 0
  done_pieces = set()
  with open(filename, 'r') as f:
    for line in f:
      ...
      if line.__contains__('test'):
        ...
        sum_loss+=loss
        print(f"{sum_loss=} {num_validation=}")
        num_validation+=1
        
  print(sum_loss/num_validation)
